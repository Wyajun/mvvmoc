...
@interface RootScrollView()<UIScrollViewDelegate>
...
-(void)setSelectIndex:(NSInteger)selectIndex {
    if(selectIndex == _selectIndex)
        return;
    if (selectIndex >= self.contentViews.count) {
        selectIndex = self.contentViews.count - 1;
    }
    if (selectIndex < 0) {
        selectIndex = 0;
    }
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
    ...
    if ([_delegate respondsToSelector:@selector(RootScrollView:selectIndex:)]) {
        [_delegate RootScrollView:self selectIndex:index];
    }
}
